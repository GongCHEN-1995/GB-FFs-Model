...
def PreProcess(mol, bond_morse=False, angle_ub=False):
...
def para_process_mol(i, all_name, dataset, GAFF, bond_morse=False, angle_ub=False, max_len=100, nb_time_processing=10, print_info=False):
    mol = []
    for j in range(i*nb_time_processing, min((i+1)*nb_time_processing,len(all_name))):
        name = all_name[j]
        one_mol = Molecule(name, dataset, GAFF, bond_morse, angle_ub, max_len, print_info=print_info)
        if one_mol.existe:
            mol.append(one_mol)
