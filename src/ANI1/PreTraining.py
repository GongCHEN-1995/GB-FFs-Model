...
#             zero the parameter gradients
        ...
        val_params_loss, val_energy_loss = Test_GAFF(model, model2, val_dl)
        test_params_loss, test_energy_loss = Test_GAFF(model, model2, test_dl)

        elapsed = time.time() - epoch_start_time
        print('| {:3d}/{:3d} | lr {:.1e} | {:2.0f} s |'.format(
                epoch, epochs, optimizer.state_dict()['param_groups'][0]['lr'], elapsed))
        print('Train params: ', epoch_loss)
        print('Val   params: ', val_params_loss)
        print('Test  params: ', test_params_loss)
        print('Val   Energy: ', val_energy_loss)
        print('Test  Energy: ', test_energy_loss)
...
#             zero the parameter gradients
        ...
        print('| {:3d}/{:3d} | lr {:.1e} | {:2.0f} s |'.format(
                epoch, epochs, optimizer.state_dict()['param_groups'][0]['lr'], elapsed))
        print('Train params: ', Params_Loss)
        print('Val   params: ', val_params_loss)
        print('Test  params: ', test_params_loss)
        print('Train Energy: ', Energy_Loss)
        print('Val   Energy: ', val_energy_loss)
        print('Test  Energy: ', test_energy_loss)
...
def Test_ANI1(model, model2, dl, print_info=False):
    model.eval()
    Loss = np.zeros(7)
    loss_fn = nn.MSELoss()
    loss_fn1 = nn.L1Loss()
        ...
        for p,data in enumerate(dl):
            ...
            for i in range(len(params_index['atom'])-1):
                ...
                if print_info and i == print_info:
                    return Energy_ref, Energy_NN, Energy_GAFF, mol_parameters, mol_targets
...
#             zero the parameter gradients
        ...
        val_Loss, val_num_conf, val_num_mol = Test_ANI1(model, model2, val_dl, print_info=False)
        torch.cuda.empty_cache()
        test_Loss, test_num_conf, test_num_mol = Test_ANI1(model, model2, test_dl, print_info=False)
        torch.cuda.empty_cache()
