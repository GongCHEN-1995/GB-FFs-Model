...
class Molecule():
    ...
    def read_ac_file(self):
        ...
        self.paras_GAFF['charge'] = np.array(charge).reshape(-1,1) * 1.1
        
    def read_smiles_file(self):
        if self.dataset == 'SPICE':
        ...
        elif self.dataset == 'DES370K':
            with h5py.File(os.path.join(self.root,'DES370K.h5'), 'r') as f:
                self.nb_shot = len(f[self.mol_name]['cbs_CCSD(T)_all'][:])
                self.smiles0 = f[self.mol_name]['smiles0'][0].decode()
                self.smiles1 = f[self.mol_name]['smiles1'][0].decode()
                self.smiles = self.smiles0 + '.' + self.smiles1
                self.nb_atom = len(f[self.mol_name]['elements'])
                self.nb_atom0 = f[self.mol_name]['natoms0'][:][0]
                self.nb_atom1 = f[self.mol_name]['natoms1'][:][0]
                assert self.nb_atom == self.nb_atom0 + self.nb_atom1
            self.atom_mapping = self.mapping_atom = np.arange(self.nb_atom).tolist()
    ...
    def read_sdf_file(self, GAFF):
        if self.dataset == 'SPICE':
            ...
            smiles0 = self.smiles0
            smiles0 = smiles0.replace("(", "L")
            smiles0 = smiles0.replace(")", "R")
            smiles0 = smiles0.replace("[", "M")
            smiles0 = smiles0.replace("]", "Q")
            
            filename0 = os.path.join(self.sdf_path,smiles0+'.sdf')
            mol0 = Chem.SDMolSupplier (filename0, removeHs=False)[0] 
            if mol0 is None:
                print('Fail to read molecule by RDKIT for SMILES0!!!', self.mol_name)
                return 1
            template0 = AllChem.MolFromSmiles(self.smiles0, sanitize=True)
            template0 = Chem.AddHs(template0)
            try:
                mol0 = AllChem.AssignBondOrdersFromTemplate(template0, mol0)
            except:
                print(self.mol_name, 'wrong assign bond order for smiles0')
                return 1
            ...
            filename1 = os.path.join(self.sdf_path,smiles1+'.sdf')
            mol1 = Chem.SDMolSupplier (filename1, removeHs=False)[0] 
            if mol1 is None:
#                 print('Fail to read molecule by RDKIT for SMILES1!!!', self.mol_name)
                return 1
            template1 = AllChem.MolFromSmiles(self.smiles1, sanitize=True)
            template1 = Chem.AddHs(template1)
            try:
                mol1 = AllChem.AssignBondOrdersFromTemplate(template1, mol1)
            except:
                print(self.mol_name, 'wrong assign bond order for smiles1')
                return 1
            
            mol = Chem.CombineMols(mol0, mol1)
            template = Chem.CombineMols(template0, template1)
        ...
        missing_para = self.get_missing_para()
        ...
        assert self.nb_atom == len(self.atom)
    ...
    def get_coord_inputs(self, gradient=False):
        ...
        if self.dataset == 'SPICE':
            ...
            with h5py.File(os.path.join(self.root,'DES370K.h5'), 'r') as f:
                target_energy_force['energy'] = f[self.mol_name]['cbs_CCSD(T)_all'][:].astype('double') 
                gradient = False
                coord = f[self.mol_name]['xyz'][:].astype('double') 
    ...
    def get_bond_list(self, mol=None, GAFF=None, missing_para=None):
        if mol is not None:
            ...
            # For DES370K dataset, we only consider non-bonded interaction
            if self.dataset == 'DES370K':
        ...
        if mol is not None:
            ...
            if self.dataset == 'SPICE':
                ...
                for i in range(len(self.angle)):
                    ...
                    if tmp not in bonded:
                        bonded.append(tmp)
                for i in range(self.nb_atom):
                    for j in range(i+1,self.nb_atom):
                ...
                for i in range(self.nb_atom0):
                    for j in range(self.nb_atom0,self.nb_atom):
                        non_bonded_list.append([i,j])
...
def PreProcess(mol, bool_ub=False):
    ...
    for i in range(len(mol)):
        ...
        coord_inputs, coord_inputs_grad, target_energy_force = mol[i].get_coord_inputs(True)
...
def construct_dataloader(mol, bool_ub=False, bool_random=False):
    dataloader = []
    ...
    for i in special_max_len:
        ...
        for j in range(num_dataloader):
            dataloader.append(PreProcess(new_mol[j * bsz: (j+1) * bsz], bool_ub))
    if bool_random:
        random.shuffle(dataloader)
    return dataloader

def para_process_mol(i, all_name, dataset, GAFF, bond_morse=False, bool_ub=False, max_len=100, nb_time_processing=10):
    mol = []
    for j in range(i*nb_time_processing, min((i+1)*nb_time_processing,len(all_name))):
        name = all_name[j]
        one_mol = Molecule(name, dataset, GAFF, bond_morse, bool_ub, max_len)
        if one_mol.existe:
            mol.append(one_mol)
